...
router.post('/login', async (req, res) => {
  ...
  if (tgId) {
    ...
    const token = jwt.sign({ id: user.id, username: user.username, tgId: user.tgId }, 'your-secret-key', { expiresIn: '1h' });
    res.json({token});
  } else {
...
router.post('/register', async (req, res) => {
  ...
  if (tgId) {
    ...
    user = { id: users.length + 1, email, username, password: hashedPassword }
    users.push(user)
  }
...
router.get('/session', (req, res) => {
  ...
  try {
    ...
    res.json({ user: { id: decoded.id, email: decoded.email, username: decoded.username, tgId: decoded.tgId } });
  } catch (error) {
